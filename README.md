...
Our functional form has population logged because it ranges from just 217 people to 9.6 million, making a one-person increase meaningless for some counties but much more important for others. Income is also logged because it varies by about a factor of six, so proportional differences in income are a more useful comparison than treating a $1 increase as having the same meaning at every income level. Smoking is not transformed because it ranges from 5.9% to 38.3% and is fairly close to symmetric. Since nothing in Figure 1 calls for transforming our main regressor, we transform where it is most logical instead.

Figures 2 and 3 also support this choice. Figure 2 shows the relationship using population in its original form, but the relationship is heavily compressed because a few very large counties pull the scale out, making it harder to see the pattern among most counties. Figure 3 uses log population, which spreads the observations out more evenly and makes the underlying relationship much clearer.
...
The F-test on the four controls jointly gives F(4, 3136) = 692.4 with a p-value below 0.001, so we reject the null that they are all zero. The controls belong as a block.

# 5. Discussion

We previously predicted a slope of 0.7 to 0.85 percentage points of fair or poor health per percentage point of smoking. The simple regression returned 0.945, and adding our controls pulled it to 0.542. The confidence interval [0.510, 0.575] sits entirely below our predicted range, so the data does not just fall short of our prediction; it rules that range out. We got the direction right and correctly predicted that the coefficient would shrink after adding controls but not collapse. However, we overestimated its size.

One variable we could not control for is physical inactivity. Counties where smoking is common plausibly also have lower rates of exercise, and inactivity independently worsens health, so it would be positively related to both our regressor and our outcome. Because it sits in the error term, the smoking coefficient picks up part of its relationship with health, which pushes our estimate of 0.542 upward. The true association is likely smaller than what we report. That is the same mechanism as the 43% shrinkage between columns (1) and (2).

We found an association across our 3,142 observed counties, **not causation**. Nothing in our analysis says that a county which lowered its smoking rate would see its health outcomes improve.
